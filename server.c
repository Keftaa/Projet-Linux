...
int main(int argc , char *argv[])
{
    ...
    while(TRUE)
    {
        ...
        if(pid > 0){
            while(TRUE){
                ...
                if (strcmp(clavier,"_shutdown")==0){
                    snprintf(lesmsg, 40, "%s","Le serveur est eteint." );
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            puts("Message de bienvenue envoye");
            ...
            for (i = 0; i < max_clients; i++)
            {
                ...
                if( client_socket[i] == 0 )
                {
                    ...
                    printf("Ajout a la liste des sockets: %d\n" , i);
        ...
        for (i = 0; i < max_clients; i++)
        {
            ...
            if (FD_ISSET( sd , &readfds))
            {
                ...
                if (((valread = read( sd , buffer, 1024)) == 0 ))
                {
                    ...
                    printf("Hote deconnecte , ip %s , port %d \n" , inet_ntoa(address.sin_addr) , ntohs(address.sin_port));
                ...
                else
                {
                    ...
                    if(egal(exp,nameinapp))
                    {
                        ...
                        for (z = 0; z < max_clients; z++)
                            ...
                            {
                                sdm = client_socket[z];
                                snprintf(lesmsg, 40, "%s","SERVER: l utilisateur ' " );
                                send(sdm , lesmsg , strlen(lesmsg) , 0 );
                                snprintf(lesmsg, 40, "%s",message );
                                send(sdm , lesmsg , strlen(lesmsg) , 0 );
                                snprintf(lesmsg, 40, "%s"," ' est connecte !\n" );
                                send(sdm, lesmsg , strlen(lesmsg) , 0 );
                        ...
                        if (egal(buffer,exp)==1)
                        {
                            ...
                            for (z = 0; z < max_clients; z++)
                                if(client_socket[z]!=0 && client_socket[z]!=sd)
                                {
                                    ...
                                    snprintf(lesmsg, 40, "%s",message );
                                    send(sdm , lesmsg , strlen(lesmsg) , 0 );
                                    snprintf(lesmsg, 40, "%s"," ' est deconnecte !\n" );
                                    send(sdm, lesmsg , strlen(lesmsg) , 0 );
...
//************************************************************************
                            ...
                            if(egal(buffer,exp)==1)
                            {
                                ...
                                if(nb>0)
                                {
                                    snprintf(lesmsg, 40, "%s","Connectes  : ");
                                    send(sd , lesmsg , strlen(lesmsg) , 0 );
                                    snprintf(lesmsg, 40, "%d",nb);
                                    send(sd , lesmsg , strlen(lesmsg) , 0 );
                                    snprintf(lesmsg, 40, "%s"," \nutilistaeur(s) connectes : ");
                                    send(sd , lesmsg , strlen(lesmsg) , 0 );
                                    m=0;
                                    for (z = 0; z < max_clients; z++)
                                        if(client_socket[z]!=0 && client_socket[z]!=sd)
                                ...
                                else {snprintf(lesmsg, 40, "%s"," vous etes en ligne seul!  ");
                                    send(sd , lesmsg , strlen(lesmsg) , 0 );}
